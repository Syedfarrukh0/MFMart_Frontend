...
const Sidebar:FC<SidebarProps> = ({selectedCategory, categories, onCategoryPress}) => {
    ...
    const indecatorPosition = useSharedValue(0);
    ...
    useEffect(()=>{
        ...
        if (targetIndex !== -1) {
            indecatorPosition.value = withTiming(targetIndex * 100, {duration: 500});
            runOnJS(()=>{
    ...
    const indicatorStyle = useAnimatedStyle(()=>({
        transform: [{translateY: indecatorPosition.value}]
    }))
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={{paddingBottom: 50}} showsVerticalScrollIndicator={false}>
            ...
            <Animated.View>
                {categories?.map((category:any, index:number)=>{
                    ...
                    return(
                        <TouchableOpacity key={index} activeOpacity={1} onPress={()=>onCategoryPress(category)} style={styles.categoryButton}>
                            <View style={[styles.imageContainer, selectedCategory?.id === category?._id && styles.selectedImageContainer]}>
